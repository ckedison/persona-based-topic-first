...
# 標題
st.title("🎯 互動式策略儀表板")
st.markdown("上傳您的 Persona，讓 AI 為您打造主題優先的內容策略")
...
with st.sidebar:
    ...
    # 匹配按鈕
    if st.button("🔍 匹配 Persona", use_container_width=True, type="primary"):
        if st.session_state.persona_df is None:
            st.warning("請先上傳 Persona 資料庫。")
        elif not topic:
            st.warning("請輸入核心主題。")
        else:
...
if st.session_state.matched_personas is not None:
    ...
    st.markdown("以下是根據您的主題匹配出的 Persona，請勾選您想為其規劃策略的對象。")
